...
| **sessions d'agent** — leur nombre, et la longueur de la plus longue | le premier poste réel dès que des agents travaillent : 65 % de la facture d'une branche mesurée le 21 août 2026. Son coût croît en **carré** de la longueur — elle fait N tours, et chacun relit ce que les N-1 précédents ont accumulé. Mesure : trois sessions de 88 à 109 tours relisaient 178 k à 238 k jetons par tour, contre 11 k à 37 k pour six sessions de 4 à 19 tours. Au-delà de `COUT_TOURS_ALERTE` — 60 tours, défini dans `scripts/cout.sh` et jamais recopié ailleurs — `pret.sh` avertit. **Couper** veut dire : deux sessions de moitié, la seconde repartant du **PRP** et non de l'exploration de la première |
| **session principale** — son nombre de tours, **cumulé sur la branche** | le seuil ne regardait que les agents, et c'est ce qui l'a rendu inopérant : sur les quatre branches `ramure-v2`, les sessions principales ont fait 470, 271, 246 et 152 tours, porté **48 % de la facture**, et rien ne s'est déclenché. Même `COUT_TOURS_ALERTE` que les agents. Le cumul vient des marqueurs `cout-releve`, **un par conteneur**, écrits dans l'entrée de journal : `cout.sh` ne voit que les conversations du conteneur courant, et il remplaçait le bloc entier à chaque passage — une branche coupée puis reprise repartait donc « courte » aux yeux des seuils. Les tours de la principale **s'additionnent** d'un conteneur à l'autre, ceux des agents prennent le **max** : couper une session en trois ne raccourcit aucun des trois morceaux |
