...
une nouvelle application de la fabrique part de celle-ci et hérite d'un
...
utilisateur non root, aucun port publié, routage dans le `compose.yaml` généré,
identité issue du seul `X-Forwarded-User` — y est tenue et vérifiable. Une application
...
- Module Go distinct sous `apps/hello-world/`, construit avec ce répertoire
  pour contexte — jamais la racine du dépôt.
...
- Nom : `hello-world`, celui de son répertoire sous `apps/`. Domaine :
  `hello-world.apps.billbob.ovh`.
...
4. **Le gabarit doit rester copiable** : un fichier de page, zéro dépendance,
   zéro asset externe. Une nouvelle application de la fabrique doit pouvoir
   partir de `apps/hello-world/` sans rien démêler.
