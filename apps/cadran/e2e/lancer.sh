#!/usr/bin/env bash
#
# lancer.sh — le bout en bout de cadran.
#
#   ./apps/cadran/e2e/lancer.sh
#
...
#
# NATIF ET NON DOCKER. cadran n'a ni base ni cache : son binaire embarque tout
# ce qu'il sert (page.html via go:embed, la base des fuseaux via time/tzdata).
# Le lancer directement plutot que de construire une image rend ce bout en
# bout jouable partout — poste de developpement sans demon Docker compris — et
# le fait passer de deux minutes a quelques secondes. Les apps qui ont
# VRAIMENT une annexe (ardoise, compteur) montent leurs conteneurs ; les
# autres n'ont pas a payer ce prix pour rien.
set -euo pipefail
cd "$(dirname "$0")"

PORT="${CADRAN_E2E_PORT:-18082}"
BIN="$(mktemp -d)/cadran"
SRV=""
...
PORT="$PORT" TZ="Europe/Paris" "$BIN" >/tmp/cadran-e2e.log 2>&1 &
SRV=$!
...
CADRAN_E2E_URL="http://localhost:$PORT" npx playwright test
